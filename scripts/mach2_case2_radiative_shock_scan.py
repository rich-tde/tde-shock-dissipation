#!/usr/bin/env python3
"""Scan the Mach2_case_2 (grey-diffusion, Skinner2019 M0=3) run and compare
every snapshot against the analytic NLTE radiative-shock solution (Lowrie &
Edwards 2008).

Skinner2019: constant gray sigma_ross = sigma_abs = 577 1/cm, ideal monatomic
hydrogen. This is a much more optically-thick, compact shock than
RadHydroMach2 (analytic profile spans roughly -0.021 to +0.003 cm), so the
sim box/duration were rescaled accordingly -- see runs/Mach2_case_2/test.cpp.
...
Same approach as mach2_radiative_shock_scan.py:
  ...
  * evaluate the analytic profile at each snapshot's own simulation time
    (v_shock is small here (~-2.4e5 cm/s) relative to v_left (~5.2e7 cm/s),
    so unlike RadHydroZhangMach5 this sim runs directly in the analytic lab
    frame -- no frame shift needed),
  * save a per-snapshot comparison figure and a summary L1-error-vs-time table.

Physical parameters must match runs/Mach2_case_2/test.cpp.

Run inside the richanalysis env:  python scripts/mach2_case2_radiative_shock_scan.py
"""
...
# ── Config ───────────────────────────────────────────────────────────────────
INPUT_DIR = "/home/hey4/RICH/alice_data/raw/Mach2_case_2"
...
TABLE_NAME = "mach2_case2_radiative_shock.txt"
...
V_LEFT = 5.19e7  # cm/s
T_LEFT = 2.18e6  # K
SIGMA_ROSS = 577.0  # 1/cm
SIGMA_ABS = 577.0  # 1/cm
...
print(f"v_shock = {solver.v_shock:.6e} cm/s (small vs v_left -- no frame shift used)")
