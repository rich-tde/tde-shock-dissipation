...
def _render_fields(
    snap, i, bbox, dims, tval, tunit, annotate, azimuth, idx, cfg, view, mask=None
):
    ...
    *view* carries the per-view geometry (box, camera zoom, scale bar, output
    ...
    for field in cfg["fields"]:
        ...
        volume_image(
            ...
            log=(rec["norm"] == "log"),
            norm=rec["norm"],
            linthresh=rec["linthresh"],
            vmin=cfg["vmins"][field],
...
def _render_evolution_frame(task):
    ...
    for view in todo:
        ...
        i, bbox, dims, tval, tunit = _index_map(
...
def _render_spin(final_path, n_evo, cfg, k_lo=0, k_hi=None):
    ...
    annotate = (
        _evolution_label(snap, final_path, cfg["days_per_tfb"])
        if cfg["annotate_time"]
        else None
    )
    mask = cfg["mask_fn"](snap) if cfg["mask_fn"] is not None else None
    angles = cfg["azimuth"] + np.linspace(
        0.0, cfg["spin_angle"], cfg["spin_frames"], endpoint=False
    )
        ...
        for field in cfg["fields"]:
            ...
            for k in range(k_lo, k_hi):
                ...
                out_png = os.path.join(
                    view["frames_dir"][field], f"frame_{n_evo + k:05d}.png"
                )
                volume_image(
                    snap,
                    field,
...
def _box_geometry(args, box, coords, tag="multi"):
    ...
    if args.scalebar:
        if pencil:
            r_p = movie_zoom.pericentre_radius(
                args.m_bh, args.m_star, args.r_star, args.beta
            )
            scalebar_frac, scalebar_label = movie_zoom.scalebar_in_rp(
                box, cam_zoom, r_p
            )
        else:
    ...
    if pencil:
        ...
        if abs(args.elevation - 90.0) > 1e-6:
            print(
                f"[{tag}] WARNING: pencil box with elevation={args.elevation} is not "
                f"face-on; the beam will be viewed edge-on.",
                flush=True,
            )
        selection_fn = functools.partial(
            movie_zoom.box_selection, coords=coords, box=box
        )
...
def main(argv=None):
    ...
    p.add_argument(
        "--box",
        default="A",
        choices=sorted(render_evolution.BOX_PRESETS),
    ...
    p.add_argument(
        "--vmins", default="", help="Per-field log/symlog vmin (comma-sep, aligned)."
    )
    ...
    p.add_argument(
        "--tag", default="g3_side", help="Output stem: <outdir>/<tag>_<field>.mp4."
    )
    p.add_argument(
        "--name-template",
        default="{tag}_{field}",
    ...
    if unknown:
        print(
            f"unknown field(s): {unknown}; known: {list(FIELD_RECIPE)}", file=sys.stderr
        )
        return 1
    ...
    for v in views:
        print(
            f"[multi] view: box={[round(b, 2) for b in v['box']]} res={v['res']} "
    ...
    print(
        f"[multi] {n} snapshots, fields={fields}, box={args.box}, "
        ...
        f"no_encode={args.no_encode}",
        flush=True,
    )
    for f in fields:
        print(
            f"[multi]   {f}: cmap={recipe[f]['cmap']} norm={recipe[f]['norm']} "
            f"weight={recipe[f]['weight']} vmin={vmins[f]} vmax={vmaxs[f]}",
            flush=True,
        )
    ...
    if not args.encode_only:
            ...
            with ctx.Pool(processes=n_jobs, maxtasksperchild=1) as pool:
                ...
                for k in range(len(tasks)):
                    try:
                        ...
                        pool.terminate()
                        print(
                            f"[multi] ABORT: no frame completed in {timeout:.0f}s after "
                            f"{k}/{len(tasks)} -- a worker was most likely OOM-killed. "
                            f"Frames on disk are kept; re-submit to resume (lower "
                            f"NJOBS if it recurs).",
                            file=sys.stderr,
                            flush=True,
                        )
                        return 1
                    print(
                        f"[multi] evolution frame {k + 1}/{len(tasks)} "
                        f"(window [{evo_lo},{evo_hi}))",
                        flush=True,
                    )
        else:
        ...
        if args.spin_frames > 0 and fstop > n:
            ...
            print(
                f"[multi] trailing spin: frames {k_lo}..{k_hi} on {snaps[-1]}",
                flush=True,
            )
    ...
    if args.no_encode:
        print(
            f"[multi] rendered frames [{fstart},{fstop}); skipping encode "
            f"(frames kept in {frames_root})",
            flush=True,
        )
        return 0
