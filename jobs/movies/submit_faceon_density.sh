...
case "${1:-}" in
  ...
  production)
    ...
    ;;
  *)
    echo "usage: bash jobs/movies/submit_faceon_density.sh preview|production" >&2
    exit 2
    ;;
esac
...
sbatch --parsable \
  "${SCHEDULER[@]}" \
  --export=ALL,QUALITY="$QUALITY",NJOBS="${NJOBS:-$DEFAULT_NJOBS}",WORKERS="${WORKERS:-$DEFAULT_WORKERS}" \
